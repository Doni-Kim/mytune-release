...
- **SQL 창 한 포맷**(2.6): 세션 상세 · Top SQL · Find SQL 이 같은 창 — 요약과 지금 이 문장을 돌리는 세션, digest 요약 표 전부와 응답 시간 분포를 보는 `[Statistics]`, 창마다 `[Excel]`. 실행 계획(TREE)의 줄에 색이 붙습니다(빨강은 조건 없는 조인 · 큰 스캔만).
